...
void cloud_cb(const sensor_msgs::PointCloud2ConstPtr &cloud_msg)
{
  ...
  {
    if ((xyz_cloud->points[points_i].z >= 0.4 && xyz_cloud->points[points_i].z < 4))
    // Assuming the min depth distance of the sensor is 0.4m and we're considering measurements up to 4m to add gaussian noise
    {
      ...
      // set values to infinity, as the octomap_server's sensor_model/max_range param will eliminate values passed this range.
      {
        xyz_cloud_filtered->points[points_i].x = INFINITY; // xyz_cloud->points[points_i].x;
        xyz_cloud_filtered->points[points_i].y = INFINITY; // xyz_cloud->points[points_i].y;
        xyz_cloud_filtered->points[points_i].z = INFINITY; // xyz_cloud->points[points_i].z;

        // ROS_INFO_STREAM("x:  " << xyz_cloud->points[points_i].x << " y:  " << xyz_cloud->points[points_i].y << " z:  " << xyz_cloud->points[points_i].z);
      }
